  ...
  try {
    ...
    newblogPost
      ...
      .then(() =>
        res.json({
          message: {
            msgBody: "New Post Posted successfully!",
            msgError: false,
          },
        })
      )
  ...
  async (req, res) => {
    try {
      ...
      blogPosts
        ...
        .then(async (bPost) => {
          bPost.cloudinaryId &&
            (await cloudinary.uploader.destroy(bPost.cloudinaryId));
          ...
          bPost
            ...
            .then(() =>
              res.json({
                message: {
                  msgBody: "Post pic ppdatedsuccessfully!",
                  msgError: false,
                },
              })
            )
  ...
  blogPosts
    ...
    .then((bPost) => {
      ...
      bPost
        ...
        .then(() =>
          res.json({
            message: { msgBody: "Post updated successfully!", msgError: false },
          })
        )
        .catch((err) => res.status(400).json("Error: " + err));
    })
...
router.delete("/delete/:postId", (req, res) => {
  blogPosts
    ...
    .then(async (deletedBlogPost) => {
      ...
      res.json({
        message: { msgBody: "Post Deleted Sucessfully", msgError: false },
      });
    })
    .catch((err) => res.status(400).json("Error: " + err));
});
...
router.put("/update/:postId/addcomm", (req, res) => {
  blogPosts
    ...
    .then((bPost) => {
      ...
      bPost.comments.push({
        //The variable names must match the model names(It's case senstive)
        username,
        postfinder,
        commBody,
        parentcommfinder,
    ...
    .then((bPost) => {
      ...
      bPost
        ...
        .then(() =>
          res.json({
            message: {
              msgBody: "Reply comment added successfuly!",
              msgError: false,
            },
          })
        )
    ...
    .then((bPost) => {
      ...
      bPost
        ...
        .then(() =>
          res.json({
            message: {
              msgBody: "Comment Updated successfully!",
              msgError: false,
            },
          })
        )
...
router.put("/update/:postId/deletecomm/:commId", async (req, res) => {
  await blogPosts
    .findById(req.params.postId)
    .then((bPost) => {
      bPost.comments = bPost.comments.filter((o) => o._id != req.params.commId);
      ...
      bPost
        ...
        .then(() =>
          res.json({
            message: {
              msgBody: "Comment deleted successfully!",
              msgError: false,
            },
          })
        )
