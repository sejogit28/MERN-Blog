...
const BlogAdminCard = (props) => {
  const deleteBlogPostFunc = () => {
    if (window.confirm("Really Delete this post?") === true) {
      var currPostTitle = prompt(
        "Please type the title of the post that you want to delete"
      );
      if (currPostTitle === props.blogpost.title) {
        props.BlogPostService.deleteBlogPost(props.blogpost._id).then(
          (data) => {
      ...
      <Card.Content>
        <Button.Group>
          ...
          <Link to={"editPic/" + props.blogpost._id}>
            <Button color="yellow">Change Pic</Button>
          </Link>
